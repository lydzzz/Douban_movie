...
# 	soup = BeautifulSoup(html, 'lxml')
# 	# Movie_url = soup.select("ol a") 获取的方式很多，这种需要去重
# 	movie_url = soup.select(".pic a", limit=1)[0].get("href")
# 	return movie_url
...
f.save(r"D:\pythonProjects\Douban_movie\豆瓣1.xls")
...
# 	url_list.append(get_all_movie(get_html(url)))
# 	one_url = get_all_movie(get_html(url))
# 	message = get_message(one_url)
# 	if message is not None:
# 		for j in range(len(row)):
# 			sheet1.write(i+1, j, message[j])
# 		f.save(r"D:\pythonProjects\Douban_movie\豆瓣copy.xls")
# 		print(message)
# 	time.sleep(3)
